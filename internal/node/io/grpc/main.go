...
func (s *serv) run() error {
  ...
  s.serv = grpc.NewServer(grpc.UnaryInterceptor(s.nodeRpc.Midleware.ServerRequestInterceptor))
...
func (s *serv) Run(ctx context.Context) {
  go func() {
    defer cfg.GetLogger().Info("f Run go1 end")
    if err := s.run(); err != nil {
      cfg.GetLogger().Error("run error", zap.Error(err))
    }
    s.finished <- true
    cfg.GetLogger().Info("f true fin")
  }()
  ...
  go func() {
    defer cfg.GetLogger().Info("f Run go2 end")
    for {
      select {
